...
public class TestFrontend {
        ...
        public void runTests() {
                ...
                System.out.print("Test 'r' load Ratings selection screen (WARNING: if pressing 'x' twice from the country selection screen does not exit app, test won't exit and run indefinitely!): ");
                if (this.testFrontendGForRatings()) {
                        System.out.println("PASSED");
                } else {
                        System.out.println("FAILED");
                }
                System.out.print("Test 'y' load year selection screen (WARNING: if pressing 'x' twice from the year selection screen does not exit app, test won't exit and run indefinitely!): ");
                if (this.testFrontendGForYears()) {
                        System.out.println("PASSED");
                } else {
                        System.out.println("FAILED");
                }
        ...
        public boolean testFrontendInitialOutputNoMovie() {
                ...
                try {
                        ...
                        ((Frontend)frontend).run(new Backend(new StringReader(
                                        "title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
                                        + "The Source of Shadows,The Source of Shadows,2020,Horror,83,USA,English,\"Ryan Bury, Jennifer Bonior\",\"Jennifer Bonior, Trevor Botkin\",Four Thieves Pro
ductions,\"Ashleigh Allard, Tom Bonington, Eliane Gagnon, Marissa Kaye Grinestaff, Jenna Heffernan, Joshua Hummel, Janice Kingsley, Chris Labasbas, Jared Laufree, Dominic Lee, Vic May, Sienna Mazz
one, Lizzie Mounter, Grace Mumm, Ashley Otis\",\"A series of stories woven together by one of our most primal fears, the fear of the unknown.\",3.5\n"
                                        + "The Insurrection,The Insurrection,2020,Action,90,USA,English,Rene Perez,Rene Perez,,\"Michael Paré, Wilma Elles, Joseph Camilleri, Rebecca Tarabocchia, J
eanine Harrington, Malorie Glavan, Danner Boyd, Michael Cendejas, Woody Clendenen, Keely Dervin, Aaron Harvey, Tony Jackson, Michael Jarrod, Angelina Karo, Bernie Kelly\",The director of the large
st media company wants to expose how left-wing powers use film to control populations.,2.9\n"
                                        + "Valley Girl,Valley Girl,2020,\"Comedy, Musical, Romance\",102,USA,English,Rachel Lee Goldenberg,\"Amy Talkington, Andrew Lane\",Sneak Preview Productions
,\"Jessica Rothe, Josh Whitehouse, Jessie Ennis, Ashleigh Murray, Chloe Bennet, Logan Paul, Mae Whitman, Mario Revolori, Rob Huebel, Judy Greer, Alex Lewis, Alex MacNicoll, Danny Ramirez, Andrew K
5.4\n"                                                                                                                                                                                              
                                ))); */
                        // set the output back to standard out for running the test
                        System.setOut(standardOut);
         ...
         * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
         * The test succeeds if the country selection screen contains all three countries
         * from the data. It fails if any of them are missing, the front end has not
         * been instantiated (is null), or there is an exception.
         * @return true if the test passed, false if it failed
         */
        public boolean testFrontendGForRatings() {
          ...
          try {
                  ...
                  // add all tests to this method
                  String appOutput = outputStreamCaptor.toString();
                  if (frontend != null && appOutput.contains("2") && appOutput.contains("3") && appOutput.contains("5")) {
        ...
        /**
         * This test runs the front end and redirects its output to a string. It then
         * passes in 'y' as a command to go to the year selection mode. It then exists
         * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
         ...
         * @return true if the test passed, false if it failed
         */
        public boolean testFrontendGForYears() {
          PrintStream standardOut = System.out;
          InputStream standardIn = System.in;
          try {
                  // set the input stream to our input (with an y to test of the program lists years)
                  String input = "y" + System.lineSeparator() + "x" + System.lineSeparator() + "x";
                  InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
                  System.setIn(inputStreamSimulator);
                  ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
                  // set the output to the stream captor to read the output of the front end
                  ...
                  // add all tests to this method
                  String appOutput = outputStreamCaptor.toString();
                  if (frontend != null && appOutput.contains("2019") && appOutput.contains("2020")) {
                          // test passes if all years from the data are displayed on the screen
                          return true;
                  } else {
                          // test failed
                          return false;
