...
public class TestMovieAndMovieDataReader {
	...
	public void runTests() {
		...
		if (this.testMovieOrder()) {
		 ...
		 if (this.testMovieRating()) {
		     System.out.println("Test MovieRating: PASSED");
		 } else {
		     System.out.println("Test MovieRating: FAILED");
		 }
		 if (this.testMovieDirector()) {
		     System.out.println("Test MovieDirector: PASSED");
		 } else {
		     System.out.println("Test MovieDirector: FAILED");
		 }
	}
	...
	public boolean testMovieOrder() {
		...
		for (MovieInterface movie : movieList) {
			if ((double)movie.getAvgVote() > lastRating) {
				// test fails
				return false;
			}
     ...
     *@return true if the test passed, false if it failed
     */
      public boolean testMovieRating() {
                List<MovieInterface> movieList;
                try {
                        movieList = readerToTest.readDataSet(new StringReader(
									      				      "title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
		...
		float Rating2 = 2.9F;
  		float Rating3 = 5.4F;
		boolean equalOne = true;
		equalOne = equalOne && (Rating1 == (movieList.get(0).getAvgVote()));
		equalOne = equalOne && (Rating2 == (movieList.get(1).getAvgVote()));
		equalOne = equalOne && (Rating3 == (movieList.get(2).getAvgVote()));		
		// test ends
		return equalOne;
	}

     /**
      *This test reads in three movies, and checks if the movies' corresponding directors are correct
      *@return true if the test passed, false if it failed
      **/
     /**
     *This test reads in three movies, then check if their ratings are correct.
     ...
     */
      public boolean testMovieDirector() {
		...
		equalOne = equalOne && (Director2.equals(movieList.get(1).getDirector()));
		equalOne = equalOne && (Director3.equals(movieList.get(2).getDirector()));		
		// test ends
		return equalOne;
	}
